...
The most straight-forward approach to this would be to use a simple Euclidian distance for step 1, and then a "diff in diff" model for step 2. However, this assumes that markets are always aligned historically, observaitons are i.i.d., and that the difference between the test and control markets is constant in the pre-intervention period. Although we prefer markets to be aligned, shifts can occur and we do not want to overpenalize occasional deviations. Moreover, the gap between the test and control markets can fluctuate – which should be factored into the uncertainty – and time series data are rarely i.i.d.
