...
lines(align$index1,align$index2)
```
The weights that are applied to the local distances are determined by the step pattern used. When the default step pattern is chosen diagonal steps are weighted by a factor of 2 while other steps receive a weight of 1. This means that a two-step "detour" will cost as much as a direct diagonal step (if all local distances are equal). Thus, this step pattern allows for as much stretching of the warping curve as possible within the specified band (see [2]). For example, in this specific case, we can match a single element of the query index (CPH) to as much as four points of the reference index (ZRH). The `dtw` package provides an easy way to visualize the step pattern:
