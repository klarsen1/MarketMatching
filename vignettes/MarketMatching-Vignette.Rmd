...
A better approach is to use *dynamic time warping* for the time series matching (step 1). This technique finds the distance along the *warping curve* – instead of the raw data – where the warping curve represents the best alignment between two time series within some user-defined constraint (time window). For the intervention analysis (step 2), the most robust approach seems to be the method implemented in the `CausalImpact` package created by Kay Brodersen at Google (see [1]). The `CausalImpact` package constructs a synthetic baseline for the post-intervention period based on a structural time series model with linear regression components based on a set of plausible control markets. 
...
However, when dealing with a large set of candidate control markets it is often prudent to trim the list before selecting the final markets and we rarely need more than three markets to create robust synthetic control series. Moreover, basing your synthetic control on markets that have a small distance in the DTW analysis boosts the face-validity of the analysis.  
...
As mentioned, the `MarketMatching` package utilizes the `CausalImpact` package written by Kay Brodersen at Google (see [1]) to do the post period inference. This package provides a robust framework that overcomes the issues with the "diff in diff" approach.
...
$$ Y_t = \mu_t + x_t \beta + e_t, e_t ~ N(0, \sigma^2_e) $$
$$ \mu_{t+1} = \mu_t + \nu_t, \nu_t ~ N(0, \sigma^2_{\nu} $$

Here $x_t$ denote the control markets and $\mu_t$ is the *local level* term. The local level term captures unobserved trends in the data and thus helps to ensure that the error residuals, $e_t$, are white noise. The linear regression term, $x_t\beta$, averages over the selected control markets.  
...
### Selecting the Local Level SE
There's no perfectly scientific way of scaling the standard error of the local level term. Using the a goodness-of-fit-based measures is not meaningful as a higher SE will always guarantee a better historical fit and the goal of the model is not to fit the data in the post-intervention period.
...
* Try different values of the standard error, and check the MAPE and Durbin-Watson statistic. This shows you the tradeoff between a larger standard errors versus sacrificing MAPE and DW. Note that the DW should be as close to 2 as possible. From here, choose a standard error that is as small as you can tolerate. 

* When you cannot make a decision, choose 0.01.

### Note on Structural Time Series Models
The `CausalImpact` package can fit much more complicated models with seasonal terms and dynamic coefficients for the linear regression component. The class of models deployed by the `CausalImpact` package is flexible and, in fact, all ARIMA models can be converted into a structural model. However, the structural model is more transparent by no operating in a differenced space and its Bayesian nature allows us to control the variance of the local level term and hence reduce the variance of the forecasted posterior values.

## How to Decide the Prior SE for the Level Term
