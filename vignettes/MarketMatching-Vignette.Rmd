...
lines(align$index1,align$index2)
```
The weights are determined by the step pattern used. Note that with the default step pattern diagonal steps are weighted by a factor of 2 while the other steps receive a weight of 1. This means that a two-step "detour" will cost as much as a direct, diagonal step (if all local distances are equal). Thus, this step pattern allows for as much stretching of the warping curve as possible within the specified band (see [2]). In this specific case, this means that we can match a single element of the query index (CPH) to as much as four points of the reference index (ZRH). The `dtw` package provides an easy way to visualize the step pattern used:

```{r, echo = TRUE, message=FALSE, eval=TRUE, fig.width=7, fig.height=5}
plot(align$stepPattern)
```

Returning our focus to the cumulative distance matrix above, we see that the total distance equals 195. This is a large number given that the total distance between Zurich and Copenhagen is 44:
...
This is *not* the most appropriate dataset to demonstrate intervention inference, as humans cannot affect the weather in the short term (long term is a different blog post). We'll merely use the data to demonstrate the features.
