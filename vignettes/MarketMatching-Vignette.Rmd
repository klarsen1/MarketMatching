...
If you ever spent time in the field of marketing analytics, chances are that you have analyzed the existence of a causal impact from a new TV campaign, a major PR event, or the entrance of a new local competitor. For these types of events we cannot track the business impact at the individual customer level; we have to analyze the impact from a bird's eye view using time series analysis at the market level (e.g., DMA, state, etc.). Data science may be changing at a fast pace, but this is a use-case that has been around forever and is still very relevant.

An intervention analysis usually involves two steps:

1. Find matching *control* markets for the *test* market where the event took place, using time series matching based on history prior to the event (the "pre period"").
2. Analyze the causal impact of the event by comparing the observed data for the test and control markets following the event (the "post period""), while factoring in differences prior to the event.

## A Traditional Approach
For the first step, the most straight-forward approach is to use the Euclidian distance to find a matching control market. However, this approach implicitly over-penalizes instances where the markets are shifted. Although it is preferable for test and control markets to be aligned consistently, occasional historical shifts should not eliminate viable control market candidates.

For the second step, the traditional approach is a "diff in diff" model where the difference between the test market and the control market is modeled. However, this assumes that observations are i.i.d. and that the historical difference between the test and control market is constant – assumptions that rarely hold true for time series data.

## A More Flexible and Robust Approach
A better approach is to use *dynamic time warping* for the time series matching step. This technique finds the distance along the *warping curve* – instead of the raw data – where the warping curve represents the best alignment between two time series within some user-defined constraints.

For the intervention analysis, the `CausalImpact` package created by Kay Brodersen at Google provides an approach that is more flexible and robust than the "diff in diff" model  (see [1]). The `CausalImpact` package constructs a synthetic baseline for the post-intervention period based on a structural time series model that leverages *multiple* matching control markets as predictors.

In summary, a more flexible and robust workflow looks like this:
...
The `MarketMatching` package implements the workflow described above by essentially providing an easy-to-use "wrapper" for the `dtw` and `CausalImpact`. Hence, the package does *not* provide any functionality that cannot be found in these packages but it simplifies the workflow of using `dtw` and `CausalImpact` together *and* provides charts and data that are easy to manipulate. R packages are simply a great way of implementing and documenting workflows.
...
lines(align$index1,align$index2)
```
Note that with the default step pattern, diagonal steps are weighted by a factor of 2 while other steps are multiplied by 1. This means that a two-step "detour" will cost as much as a direct, diagonal step – assuming all local distances are equal. Thus, this step pattern allows for as much stretching of the warping curve as possible (as long as it is within the the band). In this specific case, this means that we can match a single element of the query index (CPH) to as much as four points of the reference index (ZRH). If we want to enforce a more diagonal path, we can specify `StepPattern=symmetric1` in the `dtw` call. For a deeper explanation of how step patterns work, see [2].
